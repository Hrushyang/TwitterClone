-module(twitter_socket_engine).
-export[initiate/0].

initiate() ->
    io:fwrite("\n *********TWITTER ENGINE INITIATED***********\n"),
    ...
    socket_connection(ListenSocket, Table, Client_Socket_Mapping).

socket_connection(Listen, Table, Client_Socket_Mapping) ->
    {ok, Socket} = gen_tcp:accept(Listen),
    ok = gen_tcp:send(Socket, "Socket Connection Sucessful!!!"),
    spawn(fun() -> socket_connection(Listen, Table, Client_Socket_Mapping) end),
    server_act(Socket, Table, [], Client_Socket_Mapping).

server_act(Socket, Table, Bs, Client_Socket_Mapping) ->
        ...
        {ok, Data1} ->
            ...
            if 
                Type == "register" ->
                    ...
                    io:format("\n New Registration Initiated by USER : ~p \n", [UserName]),
                    Output = ets:lookup(Table, UserName),
                    io:format("JSON Formatted Data :: ~p\n", [Output]),
                    if
                        Output == [] ->
                            ets:insert(Table, {UserName, [{"followers", []}, {"tweets", []}]}),      
                            ets:insert(Client_Socket_Mapping, {UserName, Socket}),                
                            ok = gen_tcp:send(Socket, "User Registration Sucessful!");
                        true ->
                            ok = gen_tcp:send(Socket, "User Already Exists, Try to Signup with Different Username~n"),
                            io:fwrite("Registration Failed!\n")
                    end,
                    server_act(Socket, Table, [UserName], Client_Socket_Mapping);

                Type == "tweet" ->
                    UserName = binary_to_list(lists:nth(2, Data)),
                    Tweet = binary_to_list(lists:nth(3, Data)),
                    io:format("\n ~p sent the following tweet: ~p\n", [UserName, Tweet]),
                    ...
                    io:format("\n Tweet Sucessful New Changes: ~p\n", [Output_After_Tweet]),                  
                    responseClient(Socket, Client_Socket_Mapping, Tweet, CurrentFollowers, UserName),
                    ok = gen_tcp:send(Socket, "Tweet Processed by Server!\n"),
                    server_act(Socket, Table, [UserName], Client_Socket_Mapping);
                ...
                Type == "retweet" ->
                    ...
                    io:format("User Requested Retweet from: ~p\n", [Sub_User]),
                    Tweet = binary_to_list(lists:nth(4, Data)),
                    Out = ets:lookup(Table, Sub_User),
                    if
                        Out == [] ->
                            io:fwrite("User Not Registered Yet!\n");
                        true ->
                            ...
                            if
                                CheckTweet == true ->
                                    ...
                                    responseClient(Socket, Client_Socket_Mapping, NewTweet, CurrentFollowers, UserName);
                                true ->
                ...
                Type == "subscribe" ->
                    ...
                    if
                        Output1 == [] ->
                            io:fwrite("The username doesn't exist! \n");
                        true ->
                    ...
                    io:format("\n ~p requesting to subscribe to ~p\n", [UserName, Sub_User]),
                    
                    ok = gen_tcp:send(Socket, "Server processed subscription. Subscribed!~n"),
                    server_act(Socket, Table, [UserName], Client_Socket_Mapping);
                ...
                Type == "query" ->
                    ...
                    if
                        Option == "1" ->
                            ...
                            Tweets = searchTweetDirectory("@", Table, Sub_User, MyUserName , []),
                        ...
                        Option == "2" ->
                            ...
                            Sub_User = string:strip(Sub_UserName, right, $\n),
                            io:format("Sub_UserName: ~p\n", [Sub_User]),
                            Tweets = searchTweetDirectory("#", Table, Sub_User, Hashtag , []),
                            ok = gen_tcp:send(Socket, Tweets);
                        true ->
                            ...
                            Sub_User = string:strip(Sub_UserName, right, $\n),
                            io:format("Sub_UserName: ~p\n", [Sub_User]),
                            Tweets = tweetSearcher(Table, Sub_User),
                            ok = gen_tcp:send(Socket, Tweets)
                    end,
                    io:format("\n ~p wants to query", [UserName]),
                    server_act(Socket, Table, [UserName], Client_Socket_Mapping);
                true ->
                    io:fwrite("\n Awaiting for next command!")
            end;
...
tweetSearcher(Table, UserName) ->
    Val = ets:lookup(Table, UserName),
    Length = length(Val),
    if
        Length == 0 ->
            io:fwrite("User Not found\n"),
            ["User Isn't Registered Yet, Try inviting them!~n"];
        true ->
            Val3 = lists:nth(1, Val),
            Val2 = element(2, Val3),
            Val1 = maps:from_list(Val2),                            
            {ok, CurrentTweets} = maps:find("tweets",Val1),
            LengthTweets = length(CurrentTweets),
            if
                LengthTweets == 0 ->
                    io:fwrite("User hasn't posted any tweets\n"),
...
searchTweetDirectory(Symbol, Table, Key, Word, Found) ->
    Search = string:concat(Symbol, Word),
    if
        Key == '$end_of_table' ->
            io:fwrite("Tweets Found: ~p~n", [Found]),
            Found;
        true ->
            ...
            searchTweetDirectory(Symbol, Table, CurrentRow_Key, Word, Found1)
    end.

responseClient(Socket, Client_Socket_Mapping, Tweet, Subscribers, UserName) ->
    if
        Subscribers == [] ->
            io:fwrite("\nNo followers!\n");
        true ->
            ...
            responseClient(Socket, Client_Socket_Mapping, Tweet, Remaining_List, UserName)
    end,
    io:fwrite("Running!!!\n").
